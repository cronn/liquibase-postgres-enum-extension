...
public class RenamePostgresEnumValueChange extends AbstractPostgresEnumChange {
  ...
  public String getConfirmationMessage() {
    return "Renamed value of enum type %s from %s to %s"
        .formatted(getEnumTypeName(), getOldValue(), getNewValue());
  }
  ...
  public SqlStatement[] generateStatements(Database database) {
    return new SqlStatement[] {
      new RawParameterizedSqlStatement(
          "alter type %s rename value '%s' to '%s'".formatted(enumTypeName, oldValue, newValue))
    };
  }
}
