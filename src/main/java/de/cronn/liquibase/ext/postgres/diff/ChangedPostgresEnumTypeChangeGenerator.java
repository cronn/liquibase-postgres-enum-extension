...
public class ChangedPostgresEnumTypeChangeGenerator extends AbstractPostgresEnumTypeChangeGenerator
  ...
  public Change[] fixChanged(
    ...
    List<String> removedValues =
        comparedValues.stream().filter(value -> !referenceValues.contains(value)).toList();

    List<String> addedValues =
        referenceValues.stream().filter(value -> !comparedValues.contains(value)).toList();
    ...
    if (!removedValues.isEmpty()) {
      ...
      return new Change[] {new AddPostgresEnumValuesChange(changedObject.getName(), addedValues)};
    }
  }
}
