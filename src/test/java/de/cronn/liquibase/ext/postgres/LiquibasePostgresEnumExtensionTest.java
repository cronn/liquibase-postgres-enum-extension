...
class LiquibasePostgresEnumExtensionTest extends BaseTest {
  ...
  private final HibernateToLiquibaseDiffForPostgres differ =
      new HibernateToLiquibaseDiffForPostgres("Jane Doe");

  @Test
  void testGenerateDiff_emptyLiquibaseChangelog() {
    String diff =
        differ.generateDiff(HibernatePopulatedTestConfig.class, EmptyLiquibaseConfig.class);
  ...
  void testGenerateDiff_fullLiquibaseChangelog() {
    String diff =
        differ.generateDiff(HibernatePopulatedTestConfig.class, FullLiquibaseConfig.class);
    assertWithFile(diff, maskGeneratedId(), FileExtensions.XML);
  }
